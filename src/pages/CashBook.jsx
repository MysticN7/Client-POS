...
export default function CashBook() {
    ...
    const fetchTransactions = async () => {
        try {
            const { data } = await api.get('/cash/transactions', {
                params: filters
            });
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            if (editingTransaction) {
                await api.put(`/cash/transactions/${editingTransaction.id}`, formData);
            } else {
                await api.post('/cash/transactions', formData);
            }
            setShowForm(false);
    ...
    return (
        <div className="p-2 sm:p-4">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-4 sm:mb-6 gap-2">
                <h2 className="text-xl sm:text-2xl font-bold text-gray-800">Cash Book</h2>
                <button
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4 mb-6">
                    <div className="bg-green-50 p-4 rounded-lg border border-green-200">
                        <div className="flex items-center justify-between">
                            <div>
                                <p className="text-xs sm:text-sm text-green-600 font-medium">Total Cash In</p>
                                <p className="text-lg sm:text-2xl font-bold text-green-700">${summary.totalCashIn.toFixed(2)}</p>
                            </div>
                    ...
                    <div className="bg-red-50 p-4 rounded-lg border border-red-200">
                        <div className="flex items-center justify-between">
                            <div>
                                <p className="text-xs sm:text-sm text-red-600 font-medium">Total Cash Out</p>
                                <p className="text-lg sm:text-2xl font-bold text-red-700">${summary.totalCashOut.toFixed(2)}</p>
                            </div>
                    ...
                    <table className="w-full min-w-[700px]">
                        <thead className="bg-gray-50">
                            <tr>
                                ...
                                <th className="px-3 sm:px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Source</th>
                            ...
                            {loading ? (
                                ...
                                transactions.map((transaction) => (
                                    <tr key={transaction.id} className="hover:bg-gray-50">
                                        ...
                                        <td className="px-3 sm:px-4 py-3 text-xs sm:text-sm">{transaction.source}</td>
